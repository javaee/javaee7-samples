...
We can use the `open()` method to initialize any ressources that is required by the Reader. In our case, the initialisation is limited to copy the `payrollDataHolderBean` data to the `payrollInputRecordsIterator`.
